...
serve(async (req) => {
  ...
  try {
    ...
    // If session is invalid/expired, gracefully return not subscribed instead of error
    if (userError || !userData.user?.email) {
      logStep("Invalid or expired session - returning not subscribed", { error: userError?.message });
      return new Response(JSON.stringify({ subscribed: false }), {
        headers: { ...corsHeaders, "Content-Type": "application/json" },
        status: 200,
      });

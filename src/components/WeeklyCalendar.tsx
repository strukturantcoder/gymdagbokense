...
import { Calendar, Dumbbell, Footprints, Plus, GripVertical } from "lucide-react";
...
// Draggable workout pill component
function DraggableWorkout({ workout }: { workout: ScheduledWorkout }) {
  ...
  return (
    <div
      ...
      {...listeners}
      {...attributes}
      className={`w-6 h-6 rounded-full flex items-center justify-center relative cursor-grab active:cursor-grabbing touch-none ${
        workout.workout_type === "cardio" ? "bg-green-500/20" : "bg-primary/20"
...
function DroppableDay({
  ...
  const uncompletedWorkouts = day.scheduledWorkouts.filter(w => !w.completed_at);
  ...
  return (
    <motion.div
      ...
      className={`flex flex-col items-center p-2 rounded-lg transition-all cursor-pointer hover:ring-2 hover:ring-primary/50 min-h-[100px] ${
        isOver
    ...
    >
      ...
      <span
        className={`text-sm font-bold mb-2 ${today ? "text-primary" : ""}`}
      >
        {format(day.date, "d")}
      </span>

      <div className="flex flex-col gap-1 min-h-[24px]" onClick={(e) => e.stopPropagation()}>
        {/* Show draggable workout indicators */}
        {uncompletedWorkouts.slice(0, 2).map((workout) => (
          <DraggableWorkout key={workout.id} workout={workout} />
        ))}
        
        {/* Show completed indicators */}
        {day.completedWorkouts > 0 && !day.scheduledWorkouts.some(w => w.completed_at && w.workout_type !== "cardio") && (
        ...
        {day.completedCardio > 0 && !day.scheduledWorkouts.some(w => w.completed_at && w.workout_type === "cardio") && (
          <div className="w-6 h-6 rounded-full bg-green-500/20 flex items-center justify-center relative">
            <Footprints className="w-3 h-3 text-green-500" />
            <span className="absolute -top-1 -right-1 w-3 h-3 bg-green-500 rounded-full flex items-center justify-center text-[8px] text-white font-bold">
              ✓
        ...
        {day.scheduledWorkouts.filter(w => w.completed_at).map((workout) => (
          <DraggableWorkout key={workout.id} workout={workout} />
        ))}
...
function WorkoutDragOverlay({ workout }: { workout: ScheduledWorkout | null }) {
  if (!workout) return null;

  return (
    <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-background border-2 border-primary shadow-lg">
      {workout.workout_type === "cardio" ? (
        <Footprints className="w-4 h-4 text-green-500" />
      ) : (
        <Dumbbell className="w-4 h-4 text-primary" />
      )}
      <span className="text-sm font-medium truncate max-w-[150px]">{workout.title}</span>
...
export default function WeeklyCalendar() {
  ...
  const handleDragStart = (event: DragStartEvent) => {
    const workout = event.active.data.current as ScheduledWorkout;
  ...
  const handleDragEnd = async (event: DragEndEvent) => {
    ...
    const workout = active.data.current as ScheduledWorkout;
    ...
    setWeekDays((prev) => {
      const updated = prev.map((day) => ({
        ...day,
        scheduledWorkouts: day.scheduledWorkouts.filter((w) => w.id !== workout.id),
      }));

      const targetDayIndex = updated.findIndex(
        (d) => format(d.date, "yyyy-MM-dd") === newDate
      );
      if (targetDayIndex !== -1) {
        updated[targetDayIndex].scheduledWorkouts.push({
          ...workout,
          scheduled_date: newDate,
        });
        // Update workout type flags
        if (workout.workout_type === "cardio") {
          updated[targetDayIndex].hasCardio = true;
        } else {
          updated[targetDayIndex].hasWorkout = true;
        }
      }
    ...
    try {
      ...
      toast.success(`Flyttade "${workout.title}" till ${format(parseISO(newDate), "EEEE d/M", { locale: sv })}`);
    } catch (error) {
              ...
              <div className="grid grid-cols-7 gap-1">
                {weekDays.map((day, index) => (
                  <DroppableDay
                    ...
                    isOver={overId === format(day.date, "yyyy-MM-dd")}
                  />
                ))}
              </div>
              <DragOverlay>
                <WorkoutDragOverlay workout={activeWorkout} />
              </DragOverlay>
            </DndContext>

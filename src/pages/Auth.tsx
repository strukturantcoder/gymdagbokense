        ...
        <div className="bg-card border border-border rounded-2xl p-8 shadow-2xl">
          ...
          {isReset ? (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground text-center">{t('auth.resetDescription')}</p>
              {resetSent ? (
                <div className="p-4 bg-primary/10 border border-primary/20 rounded-lg">
                  <p className="text-sm text-foreground text-center">{t('auth.resetLinkSent')}</p>
                </div>
              ) : (
                <form onSubmit={handleResetRequest} className="space-y-4">
                  <div className="space-y-2">
                    ...
                    <div className="relative">
                      ...
                      <Input id="resetEmail" type="email" placeholder={t('auth.emailPlaceholder')} value={email} onChange={(e) => setEmail(e.target.value)} className="pl-10 bg-background border-border" />
                    </div>
                  </div>
                  <Button type="submit" className="w-full" size="lg" disabled={isLoading}>{isLoading ? t('common.loading') : t('auth.sendResetLink')}</Button>
                </form>
              )}
              <div className="text-center">
                <button type="button" onClick={() => { setIsReset(false); setResetSent(false); }} className="text-muted-foreground hover:text-foreground transition-colors text-sm">{t('auth.backToLogin')}</button>
              </div>
            </div>
          ) : (
          <>
          ...
          {!isSignUp && (
            <div className="text-center mt-3">
              <button type="button" onClick={() => setIsReset(true)} className="text-sm text-primary hover:text-primary/80 font-medium transition-colors">{t('auth.forgotPassword')}</button>
            </div>
          )}
          ...
          <Button type="button" variant="outline" className="w-full" size="lg" onClick={handleAppleSignIn} disabled={isLoading}>
            <Apple className="mr-2 h-5 w-5" />
            Logga in med Apple
          </Button>
          <div className="mt-6 text-center space-y-3">
            <p className="text-muted-foreground">{isSignUp ? t('auth.hasAccount') : t('auth.noAccount')}<button type="button" onClick={() => setIsSignUp(!isSignUp)} className="ml-2 text-primary hover:text-primary/80 font-semibold transition-colors">{isSignUp ? t('auth.login') : t('auth.createAccount')}</button></p>
            {!isSignUp && (
              <p className="text-sm">
                <button 
                  type="button" 
